use std::error::Error;

use crate::{
  data_accessors::json_ticket_id_validator, models::ticket::Ticket, ticket_serializer, user_input_acceptors::{stdin_ticket_id_getter::StdinTicketIdGetter, ticket_id_getter::TicketIdGetter}
};

use super::ticket_creating_worker::TicketCreatingWorker;

pub struct StdinTicketCreatingWorker {
  ticket_id_getter: StdinTicketIdGetter
}

impl TicketCreatingWorker for StdinTicketCreatingWorker {
  fn create_ticket(&self) -> Result<Ticket, Box<dyn Error>> {
    // stdout specific view for creating ticket
    println!("please enter a ticket id for your new ticket");
    // stdin specific user input for getting ticket id
    let ticket_id = &self.ticket_id_getter.get_ticket_id()?;

    // ticket json datastore specific ticket already exists validation
    json_ticket_id_validator::ticket_already_exists(ticket_id.as_string())?;

    let ticket = Ticket::new(**ticket_id, "Idea".to_owned());

    // ticket json datastore specific ticket saving
    let file_path = crate::data_accessors::path_provider::get_ticket_path(ticket_id.as_string());

    // stdout specific view
    println!("creating ticket");
    // ticket json datastore specific ticket saving
    ticket_serializer::serialize(&file_path, &ticket)?;
    // stdout specific view
    println!("created ticket: {}", ticket_id.as_string());

    Ok(ticket)
  }
}
